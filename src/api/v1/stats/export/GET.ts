...
export const Query = z.object({
  ...
  days: z.coerce.number().min(1).max(365).optional().default(30).describe("Number of days to export"),
...
export default new Endpoint({ Query, Output, Error, Modifier }).handle(async (input, evt) => {
  ...
  try {
    ...
    if (input.format === 'csv') {
      const csvHeaders = [
        'Link ID', 'Slug', 'Original URL', 'Title', 'Short URL', 'Domain',
        'Total Clicks', 'Clicks in Period', 'Expires At', 'Created At', 'Updated At'
      ];
